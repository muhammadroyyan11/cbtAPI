	...
	<?php 
		...
		*/
		...
		<?php 
			...
			$w1 = $rows->w1;
			$w2 = $rows->w1+$rows->w2;
			$w3 = $rows->w1+$rows->w2+$rows->w3;
			$w4 = $rows->w1+$rows->w2+$rows->w3+$rows->w4;
			$w5 = $rows->w1+$rows->w2+$rows->w3+$rows->w4+$rows->w5;
			$w6 = $rows->w1+$rows->w2+$rows->w3+$rows->w4+$rows->w5+$rows->w6;
			$w7 = $rows->w1+$rows->w2+$rows->w3+$rows->w4+$rows->w5+$rows->w6+$rows->w7;
			$w8 = $rows->w1+$rows->w2+$rows->w3+$rows->w4+$rows->w5+$rows->w6+$rows->w7+$rows->w8;
			$w9 = $rows->w1+$rows->w2+$rows->w3+$rows->w4+$rows->w5+$rows->w6+$rows->w7+$rows->w8+$rows->w9;
			$w10 = $rows->w1+$rows->w2+$rows->w3+$rows->w4+$rows->w5+$rows->w6+$rows->w7+$rows->w8+$rows->w9+$rows->w10;
			
			$tgl_now = date('M d, Y');
			$timesplit=explode(':',$rows->jam_ujian);
						...
						<div class="panel panel-default">
							...
							<div class="panel-body text-left">
								<table>
									...
									<tr><td style="width:130px;"><strong>Catatan</td><td width="20px">:</strong></td><td><?php echo $rows->catatan; ?>
		...
		<script language="javascript">	
			...
			function clickes(item) {
				...
				var no_copy = nc;
				var jml_soal = js;
				var wts = localStorage.getItem("ws"+nc)/60;
			...
			if(localStorage.getItem("ws"+nc)==='xx')
			{
				time_limit = siwa;
			...
			{
				time_limit = localStorage.getItem("ws"+nc);
